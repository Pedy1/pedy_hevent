...
elseif GetResourceState('qb-core') == 'started' or GetResourceState('qb-core') == 'starting' then
    ...
    print("^0[^1ERROR^0] Nézze meg a szerverkonzolt az információkért!^0")
end
...
if framework == 'esx' then
    ...
    AddEventHandler('esx:playerLoaded', function(xPlayer)
        if Config.TrickOrTreat then
            local houses = lib.callback.await('pedy_hevent:syncBlips', 100)
            if houses then
...
trickOrTreat = function(house)
    local canKnock = lib.callback.await('pedy_hevent:canKnock', 100, house)
    if canKnock then
        ...
        TriggerServerEvent('pedy_hevent:trickOrTreat', house, GetEntityCoords(plyPed))
        isBusy = false
    else
        lib.notify({
...
RegisterNetEvent('pedy_hevent:usePumpkin', function()
    if not pEquip then
        local ped = cache.ped
...
deletePumpkin = function(obj)
    TriggerServerEvent("pedy_hevent:deleteObj", ObjToNet(obj))
end


RegisterNetEvent('pedy_hevent:deletePumpkin', function(netObj)
    if DoesEntityExist(NetToObj(netObj)) then
        DeleteObject(NetToObj(netObj))
    end
end)

RegisterNetEvent('pedy_hevent:removeBlip', function(id)
    RemoveBlip(blips[id])
end)
